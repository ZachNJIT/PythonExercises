...
# Format command line argument as leastsquares.py datafile.data datalabels.trainlabels.x
...
if len(sys.argv) > 1:
    ...
    while ((prevError - error) > stop):
        for i in range(len(data)):
            if (lDict.get(i) == -1) or (lDict.get(i) == 1):
                d[i] = lDict.get(i) - dotProduct(data[i], w)
    ...
    print("leastsquares.py datafile.data labelfile.traininglabels.x")
