# Zach Barnhart
# Hinge Loss with Adaptive Step Size

# WARNING: RUNTIME MAY BE SEVERAL SECONDS
# Format command line argument as hinge_adaptive_eta.py datafile.data datalabels.trainlabels.x
...
def modelerror(theta, tempdata, dict):
    ...
    for i in range(len(tempdata)):
        if dict.get(i) is not None:
            derrr = 1 - dict.get(i)*(dotProduct(theta, tempdata[i]))
            if derrr < 0.0:
                derrr = 0.0
            errr += derrr
    #print(errr)
...
def gradDescent(theta, tempdata, dict, testeta):
    ...
    for j in range(len(theta)):
        ...
        for i in range(len(tempdata)):
            if dict.get(i) is not None:
                if dict.get(i)*(dotProduct(theta, tempdata[i])) < 1:
                    sum += dict.get(i)*tempdata[i][j]
...
if len(sys.argv) > 1:
    ...
    print("hinge_adaptive_eta.py datafile.data labelfile.trainlabels.x")